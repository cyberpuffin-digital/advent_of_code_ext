	...
	for freq, coords := range a.frequencies {
		...
		for index1, coord1 := range coords {
			for index2, coord2 := range coords {
				...
				var colNext, rowNext int

				// Go back
				rowNext = coord1[0]
				colNext = coord1[1]
				for next := true; next; next = a.inBounds(rowNext-ydiff, colNext-xdiff) {
					rowNext -= ydiff
					colNext -= xdiff
					if a.inBounds(rowNext, colNext) {
						a.antinodes[rowNext][colNext] = true
					}
				}
...
// inBounds checks if the coordinates are within the bounds of this antenna map
func (a *AntennaMap) inBounds(row int, col int) bool {
	if row < 0 || col < 0 {
		return false
	}
