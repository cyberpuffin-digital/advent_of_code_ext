...
// countRules scans the file to
// func (f *fileInput) countRules() (count int) {
// 	if f.ruleCount > 0 {
// 		return f.ruleCount
// 	}
...
// 	for scanner.Scan() {
// 		if ruleMatch.MatchString(scanner.Text()) {
// 			count++
// 		}
// 	}

// 	f.ruleCount = count
// 	verbosef("Found %d rules in %q.\n", count, f.filename)
...
func (f *fileInput) validateUpdates() {
	for _, update := range f.updates {
		...
		update.validate(f.rules)
		verbosef("Update valid: %t.\n", update.validOrder)
	}
